...
// forceExts is a set of extensions (with leading dot, lowercase) that bypass ignore-file filtering.
func ScanDirectory(fs afero.Fs, root string, ignoreFilePath string, forceExts []string) (int64, []string, error) {
	...
	// Build O(1) lookup map for force-extensions
	forceExtMap := make(map[string]struct{})
	for _, ext := range forceExts {
		forceExtMap[ext] = struct{}{}
	}
	...
	err = afero.Walk(fs, root, func(path string, info os.FileInfo, err error) error {
		...
		// Skip hidden files/directories by default (cannot be overridden)
		...
		// Force-ext check: for files only, bypass ignore check if extension matches
		if !info.IsDir() {
			ext := strings.ToLower(filepath.Ext(info.Name()))
			if _, forceIncluded := forceExtMap[ext]; forceIncluded {
				// Force-include: skip the ignore check and add to tree
